...
- 把 $s$ 中的 $\texttt{1}$ 变成 $\texttt{0}$，至多留一个 $\texttt{1}$ 不变，可以满足要求。操作 $\max(n-c_0-1, 0)$ 次。和 $0$ 取最大值是为了兼容 $s$ 没有 $\texttt{1}$ 的情况。特别地，如果 $s[0] = s[n-1] = \texttt{1}$，那么可以留下 $s[0]$ 和 $s[n-1]$ 不变，其余全变成 $\texttt{0}$，操作 $n-c_0-2$ 次。
