...
- 如果 $X<Y$，那么上式左侧等于 $Y$，右侧严格小于 $Y$（题目保证 $a_1 > 0$），所以 $\max(X-a_2,Y) > \max(Y-a_1,X)$，矛盾。
- 如果 $X\ge Y$，那么上式左侧两个值都 $\le X$，右侧等于 $X$，此时 $\max(X-a_2,Y) \le \max(Y-a_1,X)$ 成立。

所以当且仅当 $X\ge Y$ 成立时，$\max(X-a_2,Y)\le \max(Y-a_1,X)$ 成立。

所以当且仅当 $m_1-a_1 \ge m_2-a_2$ 成立时，先完成 $t_1$ 再完成 $t_2$ 更优（或者相同）。

这意味着，如果 $\textit{tasks}$ 中的相邻任务满足 $m_1-a_1 < m_2-a_2$，那么交换这两个任务可以让初始能量变得更小（或者不变）。于是通过交换 $\textit{tasks}$ 的相邻元素（类似冒泡排序的过程），把 $\textit{tasks}$ 按照 $m_i-a_i$ 从大到小排序，可以得到 $\textit{tasks}$ 的最优排列。
